...
**Actualizado el 2026-09-21:** el rol `PATIENT` se renombró a `CUSTOMER` en todo el código (PR #12, migración Flyway `V4`) — este documento ya usa el nombre real.
...
**16 tests en `main`**, todos en verde (`./mvnw test`). Sumando lo que está en PR abiertos sin mergear: **+4** en PR #15 (HU-22, `RegisterProfessionalServiceTest`) y **+2 archivos** en PR #14 (HU-06, dominio/validación) — 20 tests en total contando ambos.

Sin prueba directa todavía: `JwtTokenService`, `JwtAuthenticationFilter`, `AuthController`, `GlobalExceptionHandler`, `PlatformSetupService` (HU-20), `ServiceOfferingService` (la lógica de duplicado por nombre de HU-02, distinta de lo que cubre el PR #14).

`LoginServiceTest` simula el repositorio con un `Proxy` de reflexión hecho a mano en vez de Mockito. El resto de los tests nuevos (`RegisterCustomerServiceTest`, `OwnershipGuardTest`, y el `RegisterProfessionalServiceTest` del PR #15) sí usan Mockito — conviene migrar `LoginServiceTest` al mismo patrón cuando se retome.
...
| JWT construido a mano | `JwtTokenService` | Arma/parsea JSON con concatenación de strings e `indexOf`/`substring`; el `escape()` solo cubre `\` y `"`. Sigue siendo el mayor riesgo real de seguridad del código, todavía sin test |
...
6. **`ServiceOfferingService`** (HU-02) — falta cubrir la lógica de duplicado por nombre (`existsByNameIgnoreCase`), distinta de lo que ya cubre el PR #14.
7. **`PlatformSetupServiceTest`** (HU-20) — sigue sin ningún test.
8. **`JwtTokenServiceTest`** — casos límite del parseo artesanal — sigue siendo el punto de mayor riesgo real del código (sección 6). Evaluar primero si se reemplaza por una librería vetada antes de invertir esfuerzo de test (ver sección 10).
9. **`JwtAuthenticationFilterTest`** y **`AuthControllerTest`** — sin cambios, siguen pendientes.

Con los puntos 1-5 resueltos, el Quality Gate de SonarCloud del proyecto (65.2% de cobertura de código nuevo, sección 7.3) va a subir apenas se mergeen los PR #14 y #15 — son los dos que más cobertura nueva aportan de lo que queda pendiente.
...
Para cerrar Sprint 1: mergear PR #14 (HU-06) y #15 (HU-22, con el test ya agregado) para subir el Quality Gate de `main`, corregir el patrón de carrera check-then-act en `RegisterCustomerService`/`PlatformSetupService`/`ServiceOfferingService` (usando como referencia la corrección que Elena ya hizo bien en `RegisterProfessionalService`), y resolver la anomalía de HU-21 en el tablero de Azure DevOps (sigue `Active`/asignada a David pese a estar terminada).
